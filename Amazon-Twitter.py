...
def save_sample_predictions(original_texts, sample_indices, y_true, predictions, model_names):
    ...
    sample_df.to_csv('model_sample_predictions_Amazon_Twitter.csv', index=False)

def main():

    domain_specific_words_twitter = [
    # Common Twitter slang & abbreviations
    "rt", "retweet", "dm", "fomo", "idk", "lol", "lmao", "rofl", "brb",
    "omg", "wtf", "btw", "tbh", "imo", "imho", "irl", "smh",
    "afaik", "ftw", "ftl", "dm", "tl", "bf", "bff",
    
    # Common hashtags & mentions (if you want to remove)
    "hashtag", "follow", "followers", "followback", "followfriday", "ff",
    "mention", "mentions", "like", "likes", "reply", "replies",

    # Profanity and mild curse words (use with caution, adjust based on context)
    "shit", "damn", "fuck", "fucking", "bitch", "bastard", "crap", "asshole",
    "dick", "piss", "bollocks", "bloody", "bugger", "arse", "cock", "cunt",
    
    # Common Twitter phrases & filler words
    "yo", "hey", "sup", "wanna", "gonna", "gotta", "kinda", "lemme", "ain't",
    "gotta", "lemme", "cuz", "cause", "tho", "thx", "pls", "plz",

    # Other filler or frequent terms
    "tweet", "tweets", "twitter", "timeline", "trending", "viral", "trends",
    
    # URL or mentions placeholders (optional if you remove these before)
    "http", "https", "www", "com", "co", "twittercom", "tco"
]

    domain_specific_words_amazon = [
    # General Product Terms
    "product", "item", "goods", "purchase", "order",
    "package", "packaging", "shipment", "delivery", "shipping",
    
    # Quality and Condition
    "quality", "durability", "defect", "defective", "broken", "faulty",
    "damage", "warranty", "return", "refund",
    
    # Features and Specifications
    "feature", "features", "specifications", "size", "weight",
    "color", "design", "material", "performance", "functionality",
    "battery", "screen", "resolution", "sound", "speed",
    
    # Customer Experience
    "customer", "service", "support", "helpful", "satisfied",
    "recommend", "recommendation", "disappointed", "expectation",
    ...
    "discount", "deal", "sale", "offer",
    
    # Usage and Function
    "use", "used", "usage", "install", "installation",
    "compatible", "performance", "easy", "difficult", "manual",
    
    # Shipping and Delivery
    "fast", "slow", "delay", "tracking", "carrier",
    
    # Specific Product Categories (examples)
    "electronics", "appliance", "clothing", "shoes", "book",
    "beauty", "toy", "game", "furniture", "tool",
]
    ...
    df_Amazon = spark.read.csv("./Amazon.csv", header=None, inferSchema=True)

    # df_Amazon['review_cleaned'] = df_Amazon[2].apply(clean_text)
    df_Amazon['review_cleaned'] = df_Amazon[2].apply(lambda x: clean_text(x, domain_specific_words_amazon)) 
    df_Amazon['polarity_score'] = df_Amazon['review_cleaned'].apply(lexicon_sentiment_analysis)
    df_Amazon[0] = df_Amazon[0].map({1: 0, 2: 1})

    df_Twitter = pd.read_csv('./twitter_training.csv', header=None)

    # Ensure that the DataFrame has at least 3 columns (adjust the numbers if incorrect)
    if df_Twitter.shape[1] > 2:
        # Filter to only keep rows where sentiment is "Positive" or "Negative"
        df_Twitter = df_Twitter[df_Twitter[2].isin(['Positive', 'Negative'])]
        
        # Clean the text in column 3 and handle NaN by filling with an empty string
        # df_Twitter['review_cleaned'] = df_Twitter[3].apply(clean_text).fillna('')
        df_Twitter['review_cleaned'] = df_Twitter[3].apply(lambda x: clean_text(x, domain_specific_words_twitter)).fillna('')
        
        # Encode "Positive" as 1 and "Negative" as 0
        le = LabelEncoder()
        df_Twitter['sentiment_label'] = le.fit_transform(df_Twitter[2])  # Encode the sentiments
    else:
        print("Error: DataFrame does not have the expected number of columns.")

    df_Twitter['polarity_score'] = df_Twitter['review_cleaned'].apply(lexicon_sentiment_analysis)


    X_test = df_Twitter['review_cleaned']
    y_test = df_Twitter['sentiment_label']

    X_train = df_Amazon['review_cleaned']
    y_train = df_Amazon[0]

    # Polarized sentiment as regression target
    y_train_polarity = df_Amazon['polarity_score']
    y_test_polarity = df_Twitter['polarity_score']
