...
def save_sample_predictions(original_texts, sample_indices, y_true, predictions, model_names):
    ...
    sample_df.to_csv('model_sample_predictions_Amazon.csv', index=False)
...
def main():
    ...
    df = spark.read.csv("./Amazon.csv", header=None, inferSchema=True)

    # Use column indices to extract the required data
    df['review_cleaned'] = df[2].apply(clean_text)  # First column
    df['polarity_score'] = df['review_cleaned'].apply(lexicon_sentiment_analysis)
    y = df[0]  # Third column as sentiment
