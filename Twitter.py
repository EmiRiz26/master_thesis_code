...
def save_sample_predictions(original_texts, sample_indices, y_true, predictions, model_names):
    ...
    sample_df.to_csv('model_sample_predictions_Twitter.csv', index=False)
...
def main():
    ...
    if df.shape[1] > 2:
        ...
        le = LabelEncoder()
        df['sentiment_label'] = le.fit_transform(df[2])  # Encode the sentiments
    else:
    ...
    df['polarity_score'] = df['review_cleaned'].apply(lexicon_sentiment_analysis)

    # Polarized sentiment as regression target
    y_train_polarity = df.loc[X_train.index, 'polarity_score']
    y_test_polarity = df.loc[X_test.index, 'polarity_score']
